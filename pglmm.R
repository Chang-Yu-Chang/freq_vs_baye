...
# Simulate response in negative binomoa
X <- rnorm(n_species*measurements_per_species, 0, 1)
eta <- 1 + 0.8 * X + phylo_effects[sp_group] + 0.1*species_effects[sp_group]
# Set a high dispersion parameter to reduce randomness genearted by NB sampling
Y <- rnbinom(n_species * measurements_per_species, mu = exp(eta), size = 100) # dispersion parameter. var(NB) = mu + mu^2/k
